...
import {
  editStudentInitialValues,
  editStudentValidateSchema,
} from "@/components/forms/yupSchema";
import { AdminLayout } from "@/components/layout";
import { EditStudentInput } from "@/graphql/__generatedTypes__";
...
const Page: NextPageWithLayout = () => {
  ...
  const submitEditStudent = async (values: EditStudentInput) => {
    try {
      const res = await editStudent({ editStudentInput: values, studentId });
      if (res) router.back();
    } catch (error) {
  ...
  const {
    ...
    validationSchema: editStudentValidateSchema,
    onSubmit: (values) => submitEditStudent(values),
  });

  return (
    <Box padding={"2.5rem"} minWidth={"100rem"}>
      <Box>
        <PageTitleText>Edit Student</PageTitleText>
      </Box>
      <Box
        sx={{
