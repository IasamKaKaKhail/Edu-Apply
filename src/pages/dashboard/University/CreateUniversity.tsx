...
import {
  createUniInitialValues,
  createUniValidateSchema,
} from "@/components/forms/yupSchema";
import { AdminLayout } from "@/components/layout";
import { CreateUniversityData } from "@/graphql/__generatedTypes__";
...
const Page: NextPageWithLayout = () => {
  ...
  const { createUni } = useUniversity();

  const submitCreateUni = async (values: CreateUniversityData) => {
    try {
      let uploadeddocs = [];
      if (docs?.length) {
      ...
      const res = await createUni({ ...values, documents: uploadeddocs });
      if (res) router.back();
    } catch (error) {
  ...
  const {
    ...
    initialValues: createUniInitialValues,
    validationSchema: createUniValidateSchema,
    onSubmit: (values) => submitCreateUni({ ...values }),
  });
  ...
  return (
    <Box padding={"2.5rem"} minWidth={"100rem"}>
      <Box>
        <PageTitleText>Add University</PageTitleText>
      </Box>
      <Box
        sx={{
        ...
        <form noValidate onSubmit={handleSubmit}>
          ...
          >
            ...
            <InputOutlined
              ...
              value={values.email}
              name="email"
              placeholder="contact@oxford.com"
              onChange={handleChange}
            />
            <Select
              title={"Country"}
